  ...
  async function handleVerifyExperience() {
    ...
    try {
      ...
      if (isUnverifying) {
        ...
        // Regular verify flow
        const response = await verifyWorkExperience(candidateId, expIdToVerify);
...
<div class="mt-6 relative">
  ...
  >
    <div
      class="w-full flex items-center justify-between border-b-2 border-[--border] pb-2 transition-all duration-200"
    >
            ...
            >
              <div
                ...
                onclick={(e) => {
                  ...
                  if (target.closest('[role="button"]') !== e.currentTarget) {
                    return;
                  }
              ...
              >
                <span
                  class="absolute -left-1.5 top-6 h-3 w-3 rounded-full bg-black dark:bg-white group-hover:bg-gray-700 dark:group-hover:bg-gray-300 transition-colors"
                ></span>
                ...
                >
                  <div class="flex justify-between items-center">
                    ...
                    <div class="flex flex-col items-end space-y-1">
                      ...
                      {#if workExp.verifications && workExp.verifications.length > 0}
                        {@const count = workExp.verifications.length}
                        <div
                        ...
                        >
                          ...
                          {#if count === 1}
                            <span class="text-gray-500 ml-1">
                              by {workExp.verifications[0].recruiter_name ??
                                "Unknown"}
                            </span>
                          {:else if count > 1}
                            <button
                  ...
                  {#if workExp.attachments && workExp.attachments.length > 0}
                    ...
                    >
                      ...
                      View Documents ({workExp.attachments.length})
                    </button>
                  {/if}
                </div>
              </div>
